...
public class DeploymentResultConsumer {
    ...
    public void handleDeploymentResult(CommandMessage<DeploymentResultPayload> message) {
        ...
        log.info("Received deployment result for CorrelationID: {} / EventID: {}", message.getCorrelationId(), eventId);
        ...
        try {
            ...
            if (!"SUCCESS".equals(payload.getResultStatus())) {
                deploymentService.updateStatusByCorrelationId(message.getCorrelationId(), DeploymentStatus.FAILED);
                notifyClient(message.getCorrelationId(), eventId, "FAILED", null);
                return;
            }

            // 2. Worker는 성공함 -> 이제 Hub DB 업데이트 시도
            // [위험 구간] 여기서 DB 에러나 다른 예외가 발생하면 보상 트랜잭션 필요
            deploymentService.updateRunningInfo(message.getCorrelationId(), payload);
            // 3. 클라이언트 알림
            notifyClient(message.getCorrelationId(), eventId, "SUCCESS", payload.getContainerId());

        } catch (Exception e) {
            log.error("Error processing deployment result for CorrelationID: {}", message.getCorrelationId(), e);

            // [보상 트랜잭션 로직]
            // Worker는 성공했으나(컨테이너 만듦), Hub가 처리에 실패했으므로 Worker에게 "삭제(Undo)" 명령 발송
            if ("SUCCESS".equals(payload.getResultStatus())) {
                log.warn("Triggering Compensation Transaction (DELETE) for CorrelationID: {}", message.getCorrelationId());
                sendCompensationCommand(message.getCorrelationId(), payload);
            }
    ...
    // --- Helper Methods ---
    private void sendCompensationCommand(String correlationId, DeploymentResultPayload payload) {
        // 1. 보상 명령 생성 (컨테이너 삭제)
        ...
        log.info("Compensation command sent for CorrelationID: {}", correlationId);
    }

    private void notifyClient(String correlationId, String eventId, String status, String containerId) {
        notificationService.send(
                correlationId,
                eventId,
                "deployment-status",
                new DeploymentEventResponse(correlationId, status, containerId)
        );
    }
}